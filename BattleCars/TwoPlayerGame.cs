...
namespace WindowsFormsApp1
{
    public partial class TwoPlayerGame : Form
    {
        ...
        //Bitmap yellowCar = Resources.yellow;
        public TwoPlayerGame(String Player1Name, String Player2Name)
        {
            ...
            p1 = new Player(0, @"..\..\..\ProbaFeatures\yellow.png", Player1Name, 100, 100);
            p2 = new Player(180, @"..\..\..\ProbaFeatures\yellow.png", Player2Name, this.Width - 130, 100);
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            //p1 so prednica i p2 so prednica 
            if (Vector.LineSegementsIntersect(new Vector(p1.positionHoodLeft.X, p1.positionHoodLeft.Y),
                ...
                out outintersection, p2.carHeight * 2))
            {
                imaHit = true;
                p1.takeDamage(Car.frontDamage);
                p2.takeDamage(Car.frontDamage);

            }

            //p2 so prednica p1 so leva strana
                ...
                p2.carWidth * 1.5))
            {
                p1.takeDamage(Car.sideDamage);
                p2.takeDamage(Car.frontDamage);
                imaHit = true;
            }

            // p2 so prednica p1 so desna strana
                ...
                out outintersection, p2.carHeight * 2))
            {
                ...
                p1.takeDamage(Car.backDamage);
                p2.takeDamage(Car.backDamage);
                imaHit = true;
            }

            //p1 so prednica p2 so leva strana
            else if (Vector.LineSegementsIntersect(new Vector(p2.positionHoodLeft.X, p2.positionHoodLeft.Y),
                new Vector(p2.positionTrunkLeft.X, p2.positionTrunkLeft.Y),
                new Vector(p1.positionHoodRight.X, p1.positionHoodRight.Y),
                new Vector(p1.positionHoodLeft.X, p1.positionHoodLeft.Y), out outintersection, p2.carWidth * 1.5))
            {
                p1.takeDamage(Car.frontDamage);
                p2.takeDamage(Car.sideDamage);
                imaHit = true;
            }

            // p1 so prednica p2 so desna strana
            ...
            // p1 so prednica p2 od pozadi
            else if (Vector.LineSegementsIntersect(new Vector(p2.positionTrunkLeft.X, p2.positionTrunkLeft.Y),
                new Vector(p2.positionTrunkRight.X, p2.positionTrunkRight.Y),
                new Vector(p1.positionHoodRight.X, p1.positionHoodRight.Y),
                new Vector(p1.positionHoodLeft.X, p1.positionHoodLeft.Y), out outintersection, p2.carHeight * 2))
            {
                p1.takeDamage(Car.frontDamage);
                p2.takeDamage(Car.backDamage);
                imaHit = true;
            ...
            //p1 pozadi so p2 desna strana
               ...
               new Vector(p2.positionTrunkRight.X, p2.positionTrunkRight.Y),
               new Vector(p1.positionTrunkRight.X, p1.positionTrunkRight.Y),
               new Vector(p1.positionTrunkLeft.X, p1.positionTrunkLeft.Y), out outintersection, p2.carWidth * 1.5))
            {
                p1.takeDamage(Car.backDamage);
                p2.takeDamage(Car.sideDamage);
                imaHit = true;
            ...
            //p2 pozadi so p1 desna strana
               ...
               new Vector(p1.positionTrunkRight.X, p1.positionTrunkRight.Y),
               new Vector(p2.positionTrunkRight.X, p2.positionTrunkRight.Y),
               new Vector(p2.positionTrunkLeft.X, p2.positionTrunkLeft.Y), out outintersection, p1.carWidth * 1.5))
            {
                p1.takeDamage(Car.sideDamage);
                p2.takeDamage(Car.backDamage);
                imaHit = true;
            ...
            {
                {
                    if (features[i].isHit(p2.positionHoodLeft) || features[i].isHit(p2.positionHoodRight) || features[i].isHit(p2.positionTrunkLeft) ||
                        features[i].isHit(p2.positionTrunkRight))
                    {// 1 = heart; 2 = lightning; 3 = shield
                        int type = features[i].type;
                        if (type == 1)
                        {
            ...
            if (timeFornewItem == 5)
            {
                ...
                if (features.Count < 2)
                {
                    if (chosen == 1)
                    {
                        ...
                        Bitmap img = new Bitmap(@"..\..\..\ProbaFeatures\lightning.png");
                    ...
                    {
                        Point location = new Point(rnd.Next(60, this.Width - 60), rnd.Next(60, this.Height - 60));
                        Bitmap img = new Bitmap(@"..\..\..\ProbaFeatures\shield2.png");
